...
public class DataServiceMetrics {
  ...
  private final Meter m3DbHostErrorMeter;
  private final Meter influxDbErrorMeter;
  private final Meter influxDbHostErrorMeter;
  ...
  private final Meter influxDbDataPointsCount;

  private final Timer kairosDBTimer;
  private final Timer m3DBTimer;
  private final Timer influxDBTimer;
  ...
  public DataServiceMetrics(MetricRegistry metrics) {
    ...
    this.m3DbErrorMeter = metrics.meter("data-service.m3db-errors");
    this.m3DbHostErrorMeter = metrics.meter("data-service.m3db-host-errors");
    this.influxDbErrorMeter = metrics.meter("data-service.influxdb-errors");
    this.influxDbHostErrorMeter = metrics.meter("data-service.influxdb-host-errors");
    ...
    this.influxDBTimer = metrics.timer("data-service.influxdb.timer");
    ...
    this.influxDbDataPointsCount = metrics.meter("data-service.influxdb-points.written");
  ...
  public void markInfluxDbError() {
    influxDbErrorMeter.mark();
  }
  ...
  public void markInfluxDbHostErrors(long n) {
    influxDbErrorMeter.mark(n);
  }
  ...
  public void markAccount(String account, Optional<String> region, int size) {
    if (region.isPresent()) {
      getOrCreateMeter(
              accountByteMeters, "ds.acc." + account + "." + region.get() + ".check.data-rate")
          .mark(size);
      getOrCreateMeter(
              accountRateMeters, "ds.acc." + account + "." + region.get() + ".check.check-rate")
          .mark();
    } else {
  ...
  public void incInfluxDBDataPoints(long c) {
    influxDbDataPointsCount.mark(c);
  }
